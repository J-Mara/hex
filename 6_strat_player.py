#MCTS with UCT for Hex, using local template-like patterns to bias expansion.

#NOTES TO IMPROVE: Pick better templates, and make sure the bonuses make sense.
...
# --- Local template scoring --------------------------------------------------

def _score_move_templates(size: int, board: List[List[int]], move: Move, player_is_black: bool) -> float:
    """
    Compute a local heuristic score for a candidate move, based on simple
    template-like patterns within radius <= 2 of the move.

    Patterns:
    - Edge / near-edge anchor for the current player.
    - Local thickening: adjacency to own stones, triangular clusters of own stones.
    - Local attack: adjacency to opponent stones, cut-like patterns.
    - Radius-2 support from own and opponent stones.
    - Penalty for complete isolation.
    """
    r, c = move
    size_ = size

    us_val = BLACK if player_is_black else WHITE
    opp_val = WHITE if player_is_black else BLACK

    score = 0.0

    # 1. Edge / near-edge anchor
    if player_is_black:
        # Black connects top (0) to bottom (size-1)
        if r == 0 or r == size_ - 1:
            score += 1.0
        elif r == 1 or r == size_ - 2:
            score += 2.0
    else:
        # White connects left (0) to right (size-1)
        if c == 0 or c == size_ - 1:
            score += 1.0
        elif c == 1 or c == size_ - 2:
            score += 2.0

    # 2. Radius-1 neighbors
    neighbors1: List[Move] = list(_neighbors(r, c, size_))

    own_neighbors: List[Move] = []
    opp_neighbors: List[Move] = []

    for nr, nc in neighbors1:
        val = board[nr][nc]
        if val == us_val:
            own_neighbors.append((nr, nc))
        elif val == opp_val:
            opp_neighbors.append((nr, nc))

    n_own_neighbors = len(own_neighbors)
    n_opp_neighbors = len(opp_neighbors)

    # Own neighbors: local thickening
    score += 0.8 * n_own_neighbors

    #Triangular cluster of own stones: candidate + two neighbors that are adjacent
    own_neighbors_set: Set[Move] = set(own_neighbors)
    triangle_found = False
    for p in own_neighbors:
        pr, pc = p
        for q in _neighbors(pr, pc, size_):
            if q in own_neighbors_set:
                triangle_found = True
                break
        if triangle_found:
            break
    ...
    # Opponent neighbors: potential attack/block
    score += 0.5 * n_opp_neighbors

    # Cut pattern: candidate adjacent to two opponent stones that are not
    # adjacent to each other (roughly between two groups).
    cut_pattern = False
    opp_neighbors_list = opp_neighbors
    #opp_neighbors_set: Set[Move] = set(opp_neighbors_list)
    for i in range(len(opp_neighbors_list)):
        r1, c1 = opp_neighbors_list[i]
        # neighbors of the first opponent stone
        adj1 = set(_neighbors(r1, c1, size_))
        for j in range(i + 1, len(opp_neighbors_list)):
            r2, c2 = opp_neighbors_list[j]
            # if they are not neighbors of each other, treat as separate groups
            if (r2, c2) not in adj1:
                cut_pattern = True
                break
        if cut_pattern:
            break
    ...
    # 3. Radius-2 support
    radius2: Set[Move] = set()
    for nr, nc in neighbors1:
        for rr, cc in _neighbors(nr, nc, size_):
            if (rr, cc) == (r, c):
                continue
            radius2.add((rr, cc))
    ...
    # Cap counts to avoid huge scores from dense areas
    own_r2 = min(own_r2, 4)
    opp_r2 = min(opp_r2, 4)
    ...
    # 4. Isolation penalty
    if n_own_neighbors + n_opp_neighbors == 0:
        score -= 0.5
...
# --- Top-level MCTS with template-based expansion ----------------------------

def choose_move(size: int, moves: List[Move], rng: Optional[random.Random] = None, iterations: int = 100, time_limit: float = 1.0) -> Move:
    """
    Choose the next move for the current player using MCTS with UCT,
    where the expansion step is guided by simple local templates around
    each candidate move.
    ...
    for _ in range(iterations):
        ...
        # the move with the highest local template score.
