...
class PolicyMLP9:
    ...
    9x9 Hex policy:
      input  : 3*9*9 = 243
      hidden : 256 → 256
      output : 81 logits (r*9 + c)
    Uses per-colour EMA baselines to reduce variance.
    """
    def __init__(self, seed: int = 0):
        rng = np.random.default_rng(seed)
        self.W1 = rng.normal(0, 0.07, size=(243, 256)).astype(np.float32)
        self.b1 = np.zeros((256,), dtype=np.float32)
        self.W2 = rng.normal(0, 0.07, size=(256, 256)).astype(np.float32)
        self.b2 = np.zeros((256,), dtype=np.float32)
        self.W3 = rng.normal(0, 0.07, size=(256, 81)).astype(np.float32)
        self.b3 = np.zeros((81,), dtype=np.float32)
    ...
    @classmethod
    def load(cls, path: str) -> "PolicyMLP9":
