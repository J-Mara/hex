"""
Simple MCTS with UCT for Hex.

Public API:
- choose_move(size, moves, rng, iterations=1000, time_limit=1.0) -> Move
- get_last_stats() -> Optional[MCTSStats]
"""
...
class MCTSNode:
    ...
    def __init__(
        ...
        if self.is_terminal:
            ...
            occupied = set(moves)
            # all_positions is something like [(0,0), (0,1), ...]
    ...
    #def best_child(self, c_param: float = 1.41421356237) -> "MCTSNode":
        """
...
def _reward_for_winner(winner: Optional[str], root_player_is_black: bool) -> float:
    ...
    """
    if winner is None:
        print("Draw: something's gone wrong")
        return 0.5
    if winner == "Black":
    ...
    print("unexpected result: something's gone wrong")
    return 0.5
...
def _rollout(size: int, start_moves: List[Move], all_positions: Iterable[Move], root_player_is_black: bool, rng) -> float:
    ...
    - Returns the correct winner even if extra moves are played after the
      actual winning move.
    - Gives a 1-based win_turn, which we don't actually need here.
    """
    ...
    # Randomly complete the game with a single shuffle
